...
function clearHighlight() {
    ...
    if (originalMaterialsMap.size > 0) {
        originalMaterialsMap.forEach((originalMaterial, uuid) => {
            ...
            if (mesh && mesh.isMesh) {
                logDebug(`Restoring material on original mesh ${uuid}`);
                mesh.material = originalMaterial;
            } else {
                // Check user groups just in case (shouldn't be needed for original highlight)
                 userPlacedGroups.forEach(group => {
                     const userMesh = group.getObjectByProperty('uuid', uuid);
                     if(userMesh && userMesh.isMesh) {
                         logDebug(`Restoring material on user mesh ${uuid} (unexpected path)`);
                         userMesh.material = originalMaterial;
                     }
                 });
            }
        });
...
function onSelectClick(event) {
    ...
    logDebug("Intersection results:", intersects.length, intersects[0] ? `Hit: ${intersects[0].object.uuid}` : 'No hit');
    ...
    if (intersects.length > 0) {
        ...
        selectedStructureGroup.children.forEach((child, index) => {
            if (child.isMesh) {
                 logDebug(`Processing child ${index} (${child.uuid})`);
                // Store original material IF NOT ALREADY HIGHLIGHTED (precaution)
                if (child.material !== highlightMaterial && !originalMaterialsMap.has(child.uuid)) {
                    originalMaterialsMap.set(child.uuid, child.material);
                    logDebug(`Stored original material for ${child.uuid} (name: ${child.material.name})`);
                } else if (originalMaterialsMap.has(child.uuid)) {
                    logDebug(`Material for ${child.uuid} already in map.`);
                } else if (child.material === highlightMaterial) {
                    logDebug(`Child ${child.uuid} already has highlight material.`);
                }
                 ...
                 if(child.material !== highlightMaterial){
                    ...
                    logDebug(`Highlight already applied to ${child.uuid}, skipping.`);
                 }
            } else {
                logDebug(`Skipping child ${index} (${child.uuid}) - not a mesh.`);
            }
        });
        logDebug("Highlight application loop finished. Map size:", originalMaterialsMap.size);
    } else {
        logDebug("Clicked empty space or non-original brick. Selection remains cleared.");
    }
     logDebug("--- onSelectClick() Finished ---");
}
...
function onKeyDown(event) {
    ...
    if (event.key.toLowerCase() === 'c') {
        ...
        if (isMovingCloneGroup) {
            ...
            if (activeCloneGroup) {
                ...
                activeCloneGroup.children.forEach(child => {
                    if (child.isMesh && child.material.transparent) { // Only touch transparent ones
                        logDebug(`Restoring appearance for cloned child ${child.uuid}`);
                        child.material.transparent = false;
                        child.material.opacity = 1.0;
                        // Optional: Reset color/emissive if changed during cloning/transparency
                    }
                });
                ...
                logDebug("Placement complete. isMovingCloneGroup=false, activeCloneGroup=null.");
                 ...
                 // This case should ideally not happen if isMovingCloneGroup is true
            ...
            if (selectedStructureGroup) { // Check if the original group is selected
                ...
                activeCloneGroup.name = `CloneGroup_${Date.now()}`; // Give clone a unique name

                // Clone children and materials individually
                structureGroup.children.forEach((originalChild, index) => {
                     if (originalChild.isMesh) {
                         logDebug(`Cloning original child ${index} (${originalChild.uuid})`);
                         ...
                         logDebug(` -> Cloned material (name: ${clonedChild.material.name}), setting transparent`);
                         clonedChild.material.transparent = true;
                         clonedChild.material.opacity = 0.6;
                         activeCloneGroup.add(clonedChild);
                     }
                 });
                 logDebug("Finished cloning children. Cloned group children count:", activeCloneGroup.children.length);
                ...
                logDebug("'C' pressed to clone, but NO structure selected.");
            }
        }
    }
    ...
    // --- Escape Key Logic (Cancel Move or Clear Selection) ---
        ...
        if (isMovingCloneGroup && activeCloneGroup) {
            ...
            logDebug("Clearing current selection...");
...
function clearStructure() {
    ...
    while (structureGroup.children.length > 0) {
        ...
        if (child.isMesh) {
            if (child.geometry) child.geometry.dispose();
            // Material was restored by clearHighlight if needed
             if (child.material && child.material !== highlightMaterial && typeof child.material.dispose === 'function') {
                 child.material.dispose();
            } else if (child.material === highlightMaterial) {
                 // Don't dispose the shared highlight material
                 logDebug(`Note: Child ${child.uuid} still had highlight material during clear.`);
            }
        }
    ...
    if (activeCloneGroup) { // If clearing while moving, remove active clone group too
        logDebug("Clearing active clone group...");
        scene.remove(activeCloneGroup);
         activeCloneGroup.traverse(child => { /* ... disposal logic as above ... */});
        activeCloneGroup = null;
    }
    isMovingCloneGroup = false;
...
function buildStructure(structureData) {
    ...
    if(selectedStructureGroup || isMovingCloneGroup || activeCloneGroup) {
        console.warn("buildStructure called with active interaction state, attempting reset...");
        clearHighlight(); // Includes setting selectedStructureGroup = null
        isMovingCloneGroup = false;
        if (activeCloneGroup) {
            scene.remove(activeCloneGroup); // Remove any lingering clone
             activeCloneGroup.traverse(child => { /* dispose */ });
            activeCloneGroup = null;
        }
        controls.enabled = true;
    }
    ...
    while (queue.length > 0) {
        // ... (rest of build loop is unchanged, add bricks to `structureGroup`) ...
...
// --- Structure Cycling & Info Update --- (Unchanged)
function cycleStructure() { logDebug("--- cycleStructure() ---"); if(!jsonData || !Array.isArray(jsonData) || jsonData.length === 0){return;} currentJsonIndex = (currentJsonIndex + 1) % jsonData.length; if(!jsonData[currentJsonIndex] || !Array.isArray(jsonData[currentJsonIndex].data)){ console.error(`Data invalid at index ${currentJsonIndex}`); updateStructureInfo(`Error loading ${currentJsonIndex+1}`); return;} logDebug(`Cycling to ${currentJsonIndex+1}/${jsonData.length}: ${jsonData[currentJsonIndex].name||'Unnamed'}`); clearStructure(); /* <<< Call clearStructure explicitly */ buildStructure(jsonData[currentJsonIndex].data); updateStructureInfo(); logDebug("--- cycleStructure() Finished ---");}
